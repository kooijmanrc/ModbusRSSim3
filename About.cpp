...
char *pArrCreditPre = { "ModRSsim2 Version %s\t||"
      "IDB_CREDITBMP1^|"
	  ...
	  "<CTRL>-F = Save to Log File`|"			// Added <CTRL>-F on 2015-07-16 by DL
	  "<CTRL>-C = Comms/values toggle`|<CTRL>-J = insert random RS232 error-char`|"
	  "<CTRL>+<INS> = Copy Comms Window to Clipboard`||"  // Added 2016-12-26 by DL
      "IDB_CREDITBMP4^|"
      "About\r|"
      "This cool protocol test program was written by|"
      "Conrad Braam, and is released under GPL v3.|"
      " |"
...
{
   m_registeredOK = TRUE;                     // Changed from FALSE on 2016-11-06 by DL
   m_registrationTimerStart = GetTickCount();
   m_demoPeriodEnded = FALSE;
}

/*   // Removed section 2016-10-22 by DL
	...
	if (( !suppressUI ) && ( !ret ))
		{
			...
			//ShowRegistrationMessage();  // Removed 2016-10-22 by DL
		}
	return(ret);
}
...
\nit will show this message after 45 minutes, and then just try to annoy you after that.\
\n\nTo obtain a totally free key, see the Help-About window!", MB_ICONINFORMATION );
}
*/     // End of Removed section 2016-10-22 by DL
...
CString title;
   ...
   CDialog::OnInitDialog();
	...
	//m_hyperlink2.SubclassDlgItem(IDC_MODICONLINK, this);		// Deleted 2015-07-26 by DL
...
CString reg;   //registration info
   ...
   if (m_registeredOK)
	  ...
	  //reg.Format("DPIx is %d & DPIy is %d", dpiX, dpiY);
   else
...
void CAboutDlg::OnBnClickedCheckupdates()
{
   CUpdateCheck checker;
		//  Revised line below on 2015-08-05 & 2015-11-22 by DL to use www.douglyons.us for source of Version compare
        //checker.Check(CString ("http://sites.google.com/site/plcsimulator/Home/versioninfo.txt?attredirects=0&d=1")); Original Code
        // Added first, second & third  website for checking current version on 2016-07-16 & 2016-08-01 by DL
		if (!checker.Check(CString ("http://sourceforge.net/p/modrssim2/wiki/Home/")))
		if (!checker.Check(CString ("http://www.douglyons.us")))
        if (!checker.Check(CString ("http://sourceforge.net/projects/modrssim2/files/README.txt/download")))
		AfxMessageBox(IDS_UPDATE_NOCHECK, MB_OK|MB_ICONINFORMATION);   // Added MessageBox to show websites not found on 2016-07-16 by DL
}
