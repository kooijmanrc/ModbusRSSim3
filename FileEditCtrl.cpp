...
void CFECFileDialog::OnFileNameChange()
{
	...
	CWnd* wnd = GetParent();     // Updates from SourceForge courtesy of James Slootwewg 2013-10-03 (Revied 2014/12/08 by DL)
	if(wnd == NULL)
	{
		return;
	}
	UINT nfiles = CommDlg_OpenSave_GetSpec(wnd->m_hWnd, &dummy_buffer, 1);
	// End of Updates 2014/12/08. Line Below replaced by above line
    //UINT nfiles = CommDlg_OpenSave_GetSpec(GetParent()->m_hWnd, &dummy_buffer, 1);   // Old Line replaced by above
