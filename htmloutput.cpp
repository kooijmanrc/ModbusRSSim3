...
WORD aFormat = (WORD)format;
   ...
   // This next statement calls CMemoryEditorList::Format in MemoryEditorList.cpp. Move here? by DL on 2016-08-20
...
CString outputTextBuffer; 
   ...
   int closeTagLen;
      ...
      // add a refresh tag <META http-equiv="refresh" content="2,output.html">. "2" is refresh time in seconds.
      pos = outputTextBuffer.Find("<head>");    // Changed "HEAD" to "head" on 2016-08-19 by DL because HTML tags should be L/C
      if (pos>0)
      {
      CString refreshTag;
	     // Next statement deleted to simplify HTTP refresh to take out URL because this is not going to change by DL on 2016-08-19.
         // refreshTag.Format("\r\n<META http-equiv=\"refresh\" content=\"%d,%s\">", (int)(ceil(m_dwMsInterval/1000.0)), this->m_outputFileName);
         refreshTag.Format("\r\n<meta http-equiv=\"refresh\" content=\"%d\">", (int)(ceil(m_dwMsInterval/1000.0))); // Made META L/C by DL
   ...
   try{
      int sharingRetries = 10; // we try to open the file 10 times if it is locked
      DWORD errCode;
      do {
         SetLastError(0);
         ...
         if (ERROR_SHARING_VIOLATION == errCode) //still being written to when we get notification
         {
            Sleep(100); // wait long enough for writing to finish
            sharingRetries--;
         }
      } while ((ERROR_SHARING_VIOLATION == errCode) && (sharingRetries>0));
