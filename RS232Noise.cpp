...
void CRS232Noise::SetErrorTypes(CRS232Noise *pOther)
{
   ...
   // disable this feature in v7.7 until I get a chance to test it
   //m_enableFaults = FALSE;      // Commented out on 2015-Nov-30 by DL to Enable Fault Testing
}
      ...
      if (faultPercent*8 < m_errorFrequency)   // only corrupt about every 8th byte
      {
         ...
         switch(faultType)
         {
            ...
            case 3:// MISSFRAME
               ...
               {
                  *pBufferPtr++ = transmitBuffer[bufferIndex++];
                  // index character, now nuke the port.
                  NukePort();
            ...
            case 5:// PARITY
               ...
               {
                  *pBufferPtr++ = transmitBuffer[bufferIndex++];
                  // index character, now nuke the parity.
                  NukeParity();          // Changed from 'NukePort()' to 'NukeParity()' on 2015-Nov-30 by DL
...
// ------------------------------- NukeParity --------------------------- // Changed from 'NukePort()' to 'NukeParity()' on 2015-Nov-30 by DL
// configure the port for a different parity so that it transmits garbage
BOOL CRS232Noise::NukeParity()                                            // Changed from 'NukePort()' to 'NukeParity()' on 2015-Nov-30 by DL
{
BOOL error;
DCB  dcb;
...
// ------------------------------- NukePort --------------------------- // Changed from 'NukeParity()' to 'NukePort()' on 2015-Nov-30 by DL
// configure the port for a different baud so that it transmits garbage
//
BOOL CRS232Noise::NukePort()                                            // Changed from 'NukeParity()' to 'NukePort()' on 2015-Nov-30 by DL
{
BOOL error;
DCB  dcb;
