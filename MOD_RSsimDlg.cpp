...
INT ScrollPos;
CFont m_Font;  // Added 2018-09-16 by DL for setting main dialog font size

   bool IsWindows7OrGreater()
// From http://stackoverflow.com/questions/1963992/check-windows-version and Used on 2014-12-30 by DL
// USed to improve Form Minimum width calculation for Win7 Display versus previous versions
{
...
{
INT LocalDPI;
  //   The next three lines are used to determine 120 & 144 DPI screens by DL on 2016-08-14
  //   The toolbar bitmaps do not scale here and it would be better to include another set of them for these cases
  HDC screen = ::GetDC(0);                          // Added 2016-08-14 to get DPI by DL
  LocalDPI = GetDeviceCaps (screen, LOGPIXELSX);   // Added 2016-08-14 to get DPI by DL
  ::ReleaseDC (0, screen);                          // Added 2016-08-14 to get DPI by DL
return LocalDPI;
}

INT ScrnWidth()
{// This routine determines the screen width to find is we are running on a limited desktop
//  Added 2017-01-08 by DL
RECT desktop;
...
{
   "Coil Outputs   (000000)",// outputs         //Changed to show 6-digits by DL on 2015-01-09
   "Digital Inputs (100000)",// in              //Changed to show 6-digits by DL on 2015-01-09
   "Analog Inputs (300000)",   // analogs       //Changed to show 6-digits by DL on 2015-01-09
   "Holding Regs (400000)",     // registers    //Changed to show 6-digits by DL on 2015-01-09
...
BOOL   m_SaveDebugToFile;			// Save Debug Output to file flag by DL on 2015-01-10
BOOL   m_CommsTimeShow;             // Added on 2015-01-12 to send flag to RS232Port.cpp by DL
BOOL   m_CommsDecodeShow;           // Added 2016-12-28 by DL to send to RS2322Port.cpp
DWORD  m_SockTO;                    // Added on 2015-01-30 for Ethernet Socket Timeout by DL
BOOL   Initialized=FALSE;           // Added 2016-08-15 by DL
BOOL   m_longTimeouts;				// Added 2016-10-15 by DL
...
DWORD major,minor;
   ...
   // Added minor version strings to next line on 2015-07-16 by DL
   sprintf_s(lpsMyAppVersion, sizeof(lpsMyAppVersion), "%d.%d.%d.%d", HIWORD(major),LOWORD(major),HIWORD(minor),LOWORD(minor));
...
: CDialog(CMOD_simDlg::IDD, pParent)
{
   ...
   m_registerShow = TRUE;
   m_commsTimes = TRUE;               // Was FALSE, but I wanted the Times as the default. Changed 2015-01-02 by DL
   m_paused = FALSE;
   m_commsTobottom = TRUE;

   m_SaveDebugToFile = FALSE;        // Added flag and set Default by DL on 2015-01-10
   m_CommsTimeShow = TRUE;           // Added 2015-01-12 to allow RS232Port.cpp to have this flag by DL
   Initialized=FALSE;                // Added 2016-08-15 by DL
}
...
{
// Called once for every PLC Memory Value to fill CMemoryEditorList Control Box
DWORD dwValue=0;
...
BEGIN_MESSAGE_MAP(CMOD_simDlg, CDialog)
   ...
   ON_NOTIFY_EX( TTN_NEEDTEXT, 0, OnTTN_NeedText )
	...
	ON_BN_CLICKED(IDC_DEBUGWRITE, OnToggleSaveToFile)        // Added 2015-01-11 by DL
	...
	ON_BN_CLICKED(IDC_SCROLLDN, OnScrollDn)
	ON_BN_CLICKED(IDC_CLEARCOMMS, OnClearcomms)
	ON_BN_CLICKED(IDC_SHOWCOMMSTIME, OnShowcommstimeClicked)
	ON_BN_CLICKED(IDC_SHOWCOMMSDECODE, OnShowcommsDecodeClicked)
	...
	ON_COMMAND(IDH_SAVETOFILE, OnToggleSaveToFile)     // Create <Ctrl-F> Connection to Code by DL on 2015-01-10
	//}}AFX_MSG_MAP
   ON_WM_GETMINMAXINFO()
...
#endif
   ON_NOTIFY(LVN_ITEMCHANGED, IDC_DATALIST, &CMOD_simDlg::OnLvnItemchangedDatalist)
END_MESSAGE_MAP()
...
DWORD maximumOffset = GetPLCMemoryLimit(area);
   ...
   if (!IsDlgButtonChecked(IDC_CLONE))			// NOT added here by DL 2015-01-02 for correct BIT Total Column
   {
...
CMemWriteLock lk(PLCMemory.GetMutex());
   ...
   if (lk.IsLocked())
      return;
   if (!IsDlgButtonChecked(IDC_CLONE))		// NOT added here by DL 2015-Nov-18 for correct BIT Total Column Entry
   {
      for (DWORD i=0 ; i<numBits ; i++)  //gather bits in backwards
...
static CResizer::CBorderInfo s_bi[] = {
   {IDC_SERVERSTATS,     {CResizer::eFixed, IDC_MAIN, CResizer::eLeft},   // left side
    ...
    {IDC_STATIC_5,       {CResizer::eFixed, IDC_SERVERSTATS, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_SERVERSTATS, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side
                         {CResizer::eHeight,IDC_MAIN, CResizer::eBottom}},// bottom

   {IDC_LAMPRX,          {CResizer::eFixed, IDC_STATIC_5, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_STATIC_5, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side
                         {CResizer::eHeight,IDC_MAIN, CResizer::eTop}},// bottom

    {IDC_STATIC_6,       {CResizer::eFixed, IDC_LAMPRX, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_LAMPRX, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side
                         {CResizer::eHeight,IDC_MAIN, CResizer::eBottom}},// bottom

   {IDC_LAMPTX,          {CResizer::eFixed, IDC_STATIC_6, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_STATIC_6, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side
                         {CResizer::eHeight,IDC_MAIN, CResizer::eTop}},// bottom

   {IDB_TOOLBARID,       {CResizer::eFixed, IDC_MAIN, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_MAIN, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side//2017-01-07
                         {CResizer::eHeight, IDC_MAIN, CResizer::eBottom}},// bottom
   ...
   {IDC_DEBUGWRITE,      {CResizer::eFixed, IDC_MAIN, CResizer::eRight},   // left side // Added by DL on 2015-01-11
                         {CResizer::eFixed, IDC_MAIN, CResizer::eTop},    // top        // Added by DL on 2015-01-11
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side // Added by DL on 2015-01-11
                         {CResizer::eHeight,IDC_MAIN, CResizer::eBottom}},// bottom     // Added by DL on 2015-01-11
// protocol selection
   ...
   {IDC_SHOWCOMMSDECODE,  {CResizer::eFixed, IDC_MAIN, CResizer::eLeft},   // left side
                         {CResizer::eFixed, IDC_MAIN, CResizer::eTop},    // top
                         {CResizer::eWidth, IDC_MAIN, CResizer::eRight},  // right side
                         {CResizer::eHeight,IDC_MAIN, CResizer::eBottom}},// bottom
   ...
   {IDC_VERSION,         {CResizer::eFixed, IDC_TRAININGSIM, CResizer::eLeft},   // left side
                         {CResizer::eFixed, IDC_TRAININGSIM, CResizer::eTop},    // top
                         {CResizer::eProportional, IDC_TRAININGSIM, CResizer::eRight},  // right side
						 {CResizer::eHeight,IDC_TRAININGSIM, CResizer::eBottom}}, // bottom

   {IDC_SCROLLUP,        {CResizer::eProportional, IDC_STATION25, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_STATION25, CResizer::eBottom},    // top
                         {CResizer::eProportional, IDC_STATION25, CResizer::eRight},  // right side
						 {CResizer::eHeight, IDC_STATION25, CResizer::eTop}},// bottom

   {IDC_SCROLLDN,        {CResizer::eProportional, IDC_STATION25, CResizer::eRight},   // left side
                         {CResizer::eFixed, IDC_STATION25, CResizer::eBottom},    // top
                         {CResizer::eProportional, IDC_STATION25, CResizer::eRight},  // right side
						 {CResizer::eHeight, IDC_STATION25, CResizer::eTop}},// bottom
};
...
CDialog::OnInitDialog();
   ...
   // Warning Message added 2016-05-24 by DL
   if (pGlobalDialog->m_seperateRegisters)
      MessageBox("Seperate Registers for Each Station is Selected which can limit responses!", MB_OK);
   ...
   FillCharCBox(&m_dataFormatCombo, PLCDataFormatsTable, &ViewerFormatNames[1], sizeof(PLCDataFormatsTable)/sizeof(PLCDataFormatsTable[0]), m_listCtrlData.GetViewFormat());
   	...
   	// tell the contained view to re-draw the items with correct Data View Format. Added two lines below on 2014-12-30 by DL
   m_listCtrlData.SetDataViewFormat(CMemoryEditorList::ListViewFormats_(m_DataViewFormat));
   m_dataFormatCombo.SetCurSel(m_DataViewFormat - 1);		// Make CComboBox Track Actual Display on 2014-12-30 by DL
   ...
   m_lampRX.SetupLamp(600);   // Changed from 500 to 600 on 2016-Jul-04 by DL because sometime not showing activity
   m_lampTX.SetupLamp(600);   // Changed from 500 to 600 on 2016-Jul-04 by DL because sometime not showing activity
...
CString ver;   //version info
ver.Format(" Ver. %s", lpsMyAppVersion);
SetDlgItemText(IDC_VERSION, ver);                // Show version on main window

CheckDlgButton(IDC_SHOWCOMMSTIME, BST_CHECKED);  // Make TIMES checkbox show the new default of "TRUE". Changed 2015-01-02 by DL
Initialized=TRUE;   // Added 2016-08-15 by DL
OnSelchangeComboArea(); // Added 2016-08-15 by DL
...
{
   // Code below added 2016-08-18 by DL to Not call Setup Columns of ListView control until it is created.
   // Also get ListView Width is it is created and we are Initialized on the Main Control and sent it to the SetupColumns
   INT ScreenWidth = 0;
   //IDC_DATALIST	IDD_TI_SIM_DIALOG
   //CWindowRect memRect(GetDlgItem(IDC_DATALIST));
   if (GetDlgItem(IDC_DATALIST) && Initialized)
   {
   CWindowRect memRect(GetDlgItem(IDC_DATALIST));
   ScreenToClient(memRect);
   INT Left1 = memRect.left;
   INT Right1 = memRect.right;
   ScreenWidth = Right1 - Left1;
   }
   // End of 2016-08-18 DL Additions
   m_Font.DeleteObject();
   if(m_Font.CreatePointFont(100, _T("MS Sans Serif"), NULL))  // Added 2017-01-08 by DL
   GetDlgItem(IDC_DATALIST)->SetFont(&m_Font);  // Added 2017-01-08 by DL

   m_isClone = !IsDlgButtonChecked(IDC_CLONE);    // Made NOT 2014-01-02 by DL because need other programs to match this setting.
   // Modified next line 2016-08-18 by DL to send ScreenWidth of ListView box to Column Resizing functions
   m_listCtrlData.SetupColumns(LONG (cols+1), !IsClone(), ScreenWidth);  // Made NOT 2014-01-02 by DL because bits were labeled backwards.
...
DWORD animationType;
   ...
   m_numInputs  = 65536;	// Changed 2015-07-15 to max of 65536 by DL
   m_numOutputs = 65536;	// Changed 2015-07-15 to max of 65536 by DL
   m_numHolding = 65536;	// Changed 2015-07-15 to max of 65536 by DL
   m_numExtended= 65536;	// Changed 2015-07-15 to max of 65536 by DL
   m_numAnalogs = 65536;	// Changed 2015-07-15 to max of 65536 by DL
   ...
   m_DataViewFormat = 1;                 // Added Default Value on 2014-12-30 by DL
   m_registeredUser = "Completely Free"; // Added Default Value on 2015-02-01 by DL
   m_registeredKey = "66840713";         // Added Default Value on 2015-02-01 by DL
   ...
   key.Open(DRegKey::current_user, registryPath);                    // 2015-01-29 by DL was "local_machine"
   if (ERROR_SUCCESS != key.QueryValue("Port", m_portNameShort, &len))
   {
	  key.Close();
	  pGlobalDialog->AddCommsDebugString("unable to read reg. values.");
      // Remove the two lines of code below by DL on 2016-10-16 & 2016-11-06. This deprecates using HKLM for storing values.
      // This code caused Windows Server 2003 to always show the warning dialog box on startup.
      // Also it caused the registry's HKLM to have an extra entry in the APPREGISTRY_SETTINGSKEY.
      //key.Open(DRegKey::local_machine, registryPath);                 // 2015-01-29 by DL was "current_user"
   ...
   key.QueryValue("LongTimeouts", &m_longTimeouts);
   ...
   // Removed four (4) lines below on 2016-10-22 by DL
   //if (!m_registration.CheckRegistrationKey(m_registeredUser, m_registeredKey))
   //   CSplashWnd::SetWaitCount(1); // splash screen stays up longer if you are not registered
   // Changed from 9 to 1 on 2014-12-07 by DL
   //else
   ...
   key.QueryValue("DataViewFormat", &m_DataViewFormat);   // Added for Data View Format on 2014-12-30 by DL
...
DWORD animationType;
   ...
   key.SetValue("LongTimeouts", m_longTimeouts);
   ...
   key.SetValue("DataViewFormat", m_listCtrlData.GetViewFormat());	// Added 2014-12-30 by DL
   ...
   case IDM_READONLY:
      {
         ...
         else
         {
            ...
            //wndpl.showCmd = SW_MINIMIZE;  // Minimize here is annoying - Commented Out on 2015-Nov-27 by DL
...
CString csNewTitle, portName;
   ...
   if (m_SaveDebugToFile) CRS232Port::WriteToFile("Starting server(s)...");  // Added statement on 2016-12-26 by DL to save to Log file
   m_Unloading = FALSE;
   switch (protocol)
   {
      case PROTOCOL_SELMOD232 :
      case PROTOCOL_SELABMASTER232 :
      case PROTOCOL_SELAB232 :
		 m_PDUSize = 256;                  // Added as default 2017-01-02 by DL
...
BOOL CRegisterUpdaterIMP::SetAnalogs(LONG index, WORD value) // Added SetAnalogs 2015-Nov-15 by DL
{ 
   CMemWriteLock lk(PLCMemory.GetMutex());
   PLCMemory.SetAt(MODBUS_MEM_ANALOGS, index, value);

   return(true);
}															 // Added SetAnalogs 2015-Nov-15 by DL

BOOL CRegisterUpdaterIMP::SetCoils(LONG index, WORD value) // Added SetCoils 2015-Nov-16 by DL
{ 
   CMemWriteLock lk(PLCMemory.GetMutex());
   PLCMemory.SetAt(MODBUS_MEM_OUTPUTS, index, value);

   return(true);
}															 // Added SetCoils 2015-Nov-16 by DL

BOOL CRegisterUpdaterIMP::SetInputs(LONG index, WORD value) // Added SetInputs 2015-Nov-16 by DL
{ 
   CMemWriteLock lk(PLCMemory.GetMutex());
   PLCMemory.SetAt(MODBUS_MEM_INPUTS, index, value);

   return(true);
}															 // Added SetInputs 2015-Nov-16 by DL
...
static DWORD   lastTickCountOut=0, tickCount;
   ...
   if (3== nIDEvent)
   {
      if ((m_csvImportEnable) &&  // CSV import only supported for modbus!
         ((m_selectedProtocol == PROTOCOL_SELMOD232 ) || (m_selectedProtocol == PROTOCOL_SELMODETH) || (m_selectedProtocol == PROTOCOL_SELMODETH_RTU)) )

      {
         ...
         if (m_CSVImporter.HandleTimer(m_importFolder, &imp))
         {
            ...
            m_CSVImporter.UpdateRegisters(false);
         }
      }
      return;
   }
...
WORD CMOD_simDlg::PLCMemDisplayWidths(LONG typeIx)
{
   switch(m_selectedProtocol)
   {
      ...
      case PROTOCOL_SELMODETH_RTU:
         switch(typeIx)
         {
         case 0: return   16;	// Number of Columns for Outputs
            break;
         case 1: return   16;	// Number of Columns for Inputs
            break;
         case 2: return   10;	// Number of Columns for Input Registers
            break;
         case 3: return   10;	// Number of Columns for Holding Registers
            break;
         case 4: return   10;	// Number of Columns for Extended Registers
            break;
         }
         break;
      case PROTOCOL_SELABMASTER232:
...
int retCode;
   ...
   switch (m_selectedProtocol)
   {
      ...
      CServerSettings232Dlg dlg;
		 ...
		 dlg.m_longTimeouts = m_longTimeouts;
         ...
         if (IDOK==retCode)
         {
			...
			m_longTimeouts = dlg.m_longTimeouts;
            m_rts          = dlg.m_rts;
      ...
      CServerSettingsEthDlg dlg;
         ...
         dlg.m_SockTO    = m_SockTO;            // 2015-01-30 by DL changed from "localID" to "SockTO" for Socket Timeout
         dlg.m_responseDelay = m_responseDelay;
         ...
         if (IDOK == retCode)
         {
            ...
            m_SockTO          = dlg.m_SockTO;   // 2015-01-30 by DL changed from "localID" to "SockTO" for Socket Timeout
            m_responseDelay   = dlg.m_responseDelay;
...
if (!Initialized) return;   // Added because of OnResize faults early in initializaion on 2016-08-15 by DL
...
{
    // if (PLCIsBitMemory(m_listCtrlData.GetAreaDisplayed()))  // Removed 2014-12-08 by DL
	// This will cause a redraw now of any type of I/O when the Clone box is changed
	// I tried just removing the "IF" statement, but I got errors about
	// local function definitions are illegal

	if (TRUE)  // Added 2014-12-08 by DL
...
CRS232Noise *pNoiseObject = NULL;
   ...
   if (IDOK==faultDlg.DoModal())
   {
    ...
    //m_ToolBar.SetButtonInfo(5, IDC_NOISE,  TBBS_BUTTON, (faultDlg.m_errorFrequency?11:10)); //bitmaps 10,11   // Deleted 2015-Nov-30 by DL
      m_ToolBar.SetButtonInfo(5, IDC_NOISE,  TBBS_BUTTON, (faultDlg.m_errors?11:10));       //bitmaps 10,11     // Added 2015-Nov-30 by DL
   }
   SaveApplicationSettings();
} // OnNoise
...
CString bookMark = m_htmBookmark;
   ...
   if (ResourceToFile(IDR_HELPHTM, fileName))
   {
      ...
      sprintf(fileName, "%s\\images\\regsta2.gif", currentfolder);
      ResourceToFile(IDR_REGSTA, fileName);
	  sprintf(fileName, "%s\\images\\advemul.gif", currentfolder);
      ResourceToFile(IDR_ADVEMUL, fileName);
	  sprintf(fileName, "%s\\images\\advemul2.gif", currentfolder);
      ResourceToFile(IDR_ADVEMUL2, fileName);
...
void CMOD_simDlg::OnToggleDisplay() 
{
   ...
   if (m_registerShow)
   {
      ...
      GetDlgItem(IDC_SHOWCOMMSDECODE)->ShowWindow(SW_HIDE);
   ...
   else
   {
      ...
      GetDlgItem(IDC_SHOWCOMMSDECODE)->ShowWindow(SW_SHOW);
...
int pos;
   ...
   {
   SYSTEMTIME sysTime;
      // Fetch and put in time HH:MM:SS:sss for all things on this screen
      // Time here is the time that the event comes into the Que, not the time it gets displayed,
      // so pausing the display does not affect the time shown!
      GetLocalTime(&sysTime);
      entry.Format("%02d:%02d:%02d.%03d %s", sysTime.wHour, sysTime.wMinute, sysTime.wSecond,
		  sysTime.wMilliseconds, string);   // Added Hours on 2015-01-11 by DL
   }
   else
      entry = string;
...
int lastTick;
   ...
   if (lastTick>STATIONTICKBOXESMAX)      // Removed "=" by DL on 2016-04-22 to show active station #255
      lastTick = STATIONTICKBOXESMAX;     // Removed "-1" by DL on 2016-04-22 to show active station #255
   return(lastTick);
}
...
int stationIndex;
   ...
   lastTick = GetLastVisibleStation();
//   CString msg;
//      msg.Format("ScrollPos is '%d', firstTick is '%d', and lastTick is '%d'", ScrollPos, firstTick, lastTick);
//      MessageBox(msg);

// Left for Debugging example
// CString msg;    // 2016-04-22 Added by DL
// msg.Format("Last Visible Station is %d?", lastTick);    // 2016-04-22 Added by DL
// AddCommsDebugString(msg);    // 2016-04-22 Added by DL
...
void CMOD_simDlg::OnSize(UINT nType, int cx, int cy) 
{
  ...
  m_resizer.Move();
   ...
   if (m_ToolBar.m_hWnd && GetDlgItem(IDC_PROTOCOL))
   {
      CWindowRect toolRect(GetDlgItem(IDB_TOOLBARID));  // Find the Toolbar
      ScreenToClient(toolRect);
	  
	  CWindowRect protRect(GetDlgItem(IDC_PROTOCOL));   // Find the Protocol Box
      ScreenToClient(protRect);

      toolRect.left = protRect.right - 175;  // Toolbar Left side = Prococol box right side less 175
   ...
   OnSelchangeComboArea();   // Added to allow auto column resizing for registers on 2016-08-15 by DL
}
...
{
INT horizontal = ScrnWidth();           // Added 2017-01-08 by DL to check screen width
	if (IsWindows7OrGreater())			// Added to change for Win7 2015-01-02 by DL
	   if (horizontal >= 825)           // Added 2017-01-08 by DL to see if our screen is less that desired
	    lpMMI->ptMinTrackSize.x = 825;  //  Revised 2015-01-08 & 2015-07-16 & 2017-01-08 by DL
	   else                                         // Added 2017-01-08 by DL
		lpMMI->ptMinTrackSize.x = horizontal - 20;  // Added 2017-01-08 by DL to handle small screens
	else
	    lpMMI->ptMinTrackSize.x = 750;  //  Revised 2015-01-08 & 2015-07-16 by DL
	                                    // End of change for Win7 2015-01-02 by DL.
   // Return the minimum window size I allow
   //lpMMI->ptMinTrackSize.x = 640;	        // Commented Out 2014-12-30 by DL
...
// ----------------------------------- OnScroll Down -----------------------------
// Scroll Down Button
void CMOD_simDlg::OnScrollDn() 
{
    ScrollPos = ScrollPos + 1;
    if (ScrollPos > MAX_SCROLLPAGES) ScrollPos = MAX_SCROLLPAGES;
    ConfigureStationButtons();
}
...
void CMOD_simDlg::OnShowcommstimeClicked() 
{
   ...
   if (IsDlgButtonChecked(IDC_SHOWCOMMSTIME))
      ...
      m_commsTimes = FALSE;
   m_CommsTimeShow = m_commsTimes;              // Added 2015-01-12 by DL to allow RS232Port.cpp to track this flag
   LeaveCriticalSection(&debuggerCritSection);
}

// ------------------------- OnShowcommsDebugClicked ------------------------
// Show event Debug on all "comms debug log" toggle
void CMOD_simDlg::OnShowcommsDecodeClicked() 
{
   EnterCriticalSection(&debuggerCritSection);

   if (IsDlgButtonChecked(IDC_SHOWCOMMSDECODE))
      m_commsDecode = TRUE;
   else
      m_commsDecode = FALSE;
   m_CommsDecodeShow = m_commsDecode;              // Allow RS232Port.cpp to track this flag
   LeaveCriticalSection(&debuggerCritSection);
}
...
CCSVFileImportDlg dlg;
   ...
   if (IDOK == dlg.DoModal())
   {
	  if (dlg.m_csvImportEnable)                                                          //Added 2015-Nov-21 by DL
	  {                                                                                   //Added 2015-Nov-21 by DL
	    CString msg;                                                                      //Added 2015-Nov-21 by DL
        msg.Format("Auto CSV Import is ON and the folder is '%s'", dlg.m_importFolder);   //Added 2015-Nov-21 by DL
	    AddCommsDebugString(msg);                                                         //Added 2015-Nov-21 by DL
        //AddCommsDebugString(dlg.m_importFolder);                                        //Deleted 2015-Nov-21 by DL
        msg.Format("The CSV Log file is '%s'", dlg.m_logFileName);                        //Added 2015-Nov-21 by DL
	    AddCommsDebugString(msg);                                                         //Added 2015-Nov-21 by DL
        //AddCommsDebugString(dlg.m_logFileName);                                         //Deleted 2015-Nov-21 by DL
	  }                                                                                   //Added 2015-Nov-21 by DL
	  if (!dlg.m_csvImportEnable)                                                         //Added 2015-Nov-21 by DL
	  {                                                                                   //Added 2015-Nov-21 by DL
		 CString msg;                                                                     //Added 2015-Nov-21 by DL
	     msg.Format("Auto CSV Import is OFF");                                            //Added 2015-Nov-21 by DL
		 AddCommsDebugString(msg);                                                        //Added 2015-Nov-21 by DL
	  }                                                                                   //Added 2015-Nov-21 by DL
...
void CMOD_simDlg::OnLvnItemchangedDatalist(NMHDR *pNMHDR, LRESULT *pResult)
{
...
/////////////////////////////////////////////////////////////////////////////////
// The next section handles the toggling of the Save To File of Debug Info flag.
// Created by DL on 2015-01-10
// -------------------------------- OnToggleDebugToFile ---------------------------
// hotkey CTRL-F
void CMOD_simDlg::OnToggleSaveToFile() 
{
	m_SaveDebugToFile = !m_SaveDebugToFile;
	if (m_SaveDebugToFile)
       CheckDlgButton(IDC_DEBUGWRITE, BST_CHECKED);
	else
       CheckDlgButton(IDC_DEBUGWRITE, BST_UNCHECKED);
}
