...
CHAR * PLCViewerFormats[8] =
{
   ...
   "%u",    //   VIEWFORMAT_DWORD,
   "%ld",   //   VIEWFORMAT_LONG,
   "%g",    //   VIEWFORMAT_FLOAT         // Changed from %f to %g by DL on 2015-01-05
   "%c"     //   VIEWFORMAT_CHAR
};
...
extern INT  DPIx();                  // Added by DL on 2016-08-14

WORD WIDTH_LISTCOLS = 70;		// Default Column Width. Moved from .h file here by DL on 2015-01-05
WORD WIDTH_LISTCOL1 = 100;		// Default First Column Width, Moved from .h file here by DL on 2015-01-05
WORD WIDTH_LISTCOLBIT = 42;		// if col is a bit, make it narrower. Moved from .h file here by DL on 2015-07-15
...
int ave = (256-(int)((red+blue+green)/6.0));
   ...
   m_backSysColorFade = RGB(200,200, 200);		// Correction for "1" in digitals show up "Black" by DL on 2015-01-02
...
BOOL CMemoryEditorList::Is32BitFormat(ListViewFormats_ format)
{
   ...
   case VIEWFORMAT_HEX:				// Added DL 2014/12/11 to allow HEX input; otherwise assert(0)
   case VIEWFORMAT_CHAR:			// Added DL 2014/12/11 to allow CHAR input; otherwise assert(0)
      return(FALSE);
      break;
   default:
      ASSERT(0);
...
void CMemoryEditorList::SetupColumns(LONG width, BOOL cloneAddressing, INT ScreenWidth)
{
int nColumnCount;

//if (IsWindows7OrGreater())      // Added to change for Win7 2015-01-05 by DL
if (DPIx() > 96)        // Chenaged to check for DPI 2016-08-14 by DL
{
    WIDTH_LISTCOLS = 74;		// Was 80 for Registers
    WIDTH_LISTCOL1 = 130;		// Was 125
	WIDTH_LISTCOLBIT = 46;		// Was 42
}
else
{
	WIDTH_LISTCOLS = 64;		// Was 56 for Registers
	WIDTH_LISTCOL1 = 105;		// Was 100
	WIDTH_LISTCOLBIT = 40;		// Was 42
}                                  // End of change for Win7 2015-01-05 by DL
   ...
   {   
      ...
      if (pGlobalDialog->PLCIsBitMemory(m_memAreaDisplayed))
      {
         ...
         for (i=0 ; i<=15 ; i++)
         {
            if (!cloneAddressing)                        // Added NOT on 2015-01-02 by DL for BIT number corrections
      ...
      for(i = 0; i < (int)m_displayableWidth+1; i++)
      {
         ...
         if (i!=0)
         {
            ...
            {
			   int ScrollWidth = GetSystemMetrics(SM_CXVSCROLL);                    // Added 2016-08-15 by DL
               // col width for a register
			   INT NewWidth = (ScreenWidth - WIDTH_LISTCOL1 - ScrollWidth - 5)/10;  // Added 2016-08-15 by DL
			   lvColumn.cx = NewWidth;                                              // Added 2016-08-15 by DL
               //lvColumn.cx = WIDTH_LISTCOLS;                                      // Deleted 2016-08-15 by DL
            } 
         }
...
// Called only in htmloutput.cpp for formatting output values. Description by DL on 2016-08-19
{
LONG  *pLongVal;
float *pFloatVal;
CString formatting;
TCHAR szString[MAX_PATH];                           // Added 2016-08-19 by DL for char output

   formatting = PLCViewerFormats[format];
   switch (format)
   {
   //case CMemoryEditorList::VIEWFORMAT_DECIMAL:    // Moved to below on 2016-08-18 by DL
   case CMemoryEditorList::VIEWFORMAT_LONG:
	  pLongVal = (LONG*)&memoryValue;
      text.Format(formatting, *pLongVal);           // Changed pLongVal to *pLongVal on 2016-08-19 by DL
      break;
   case CMemoryEditorList::VIEWFORMAT_DECIMAL:      // Moved from Above on 2016-08-18 by DL
      text.Format(formatting, (short)memoryValue);  // Added (short) to handle +/- on 2016-08-19 by DL
      break;
   ...
   case CMemoryEditorList::VIEWFORMAT_FLOAT:
      ...
      text.Format(formatting, *pFloatVal);          // Changed pFloatVal to *pFloatVal on 2016-08-19 by DL
      break;
   case VIEWFORMAT_CHAR:                                  // Added 2016-08-19 by DL for char output
      ConvertWordToASCII(szString, (WORD)memoryValue);    // Added 2016-08-19 by DL for char output
	  text = szString;                                    // Added 2016-08-19 by DL for char output
      break;                                              // Added 2016-08-19 by DL for char output
   default:
      text = "???";                                 // This handles simple mistakes for Format Types
      ASSERT(0);
      break;
   }
}
...
// **** 20021101 mod by HF end
      ...
      if ((pGlobalDialog->m_seperateRegisters) && 
          ((PROTOCOL_SELMOD232 == pGlobalDialog->m_selectedProtocol)||
           ...
           (PROTOCOL_SELMODETH_RTU == pGlobalDialog->m_selectedProtocol)))
      {  // work out which station and register # this item belongs to if we are using seperate modbus reg. /station
         registerName.Format("Station # %d , Address %ld",			// Changed "I/O" to "Address" on 2015-07-18 by DL
                     (WORD)(absItemNumber / pGlobalDialog->m_numSeperate) + 1, // Added +1 on 2017-01-05 by DL
                     (DWORD)(memPrefix*100000 + (absItemNumber % pGlobalDialog->m_numSeperate)+1));
						// Added memPrefix adjustment and +1 on 2015-07-18 by DL
      }
      else
      ...
      OutputDebugString(description);
               ...
               if (IDOK==memEditor.DoModal())
               {
                  ...
                  {
                  DWORD dwValueToSave = (DWORD) memEditor.m_value;           // Added cast for right side to avoid warning on 2016-07-14 by DL
                     if (CMemoryEditorList::VIEWFORMAT_FLOAT == GetViewFormat())
                     {
                        ...
                        // Comes here after entry box for value pops - to save value
                     }
...
// Removed 2015-01-01 by DL because it was found that this was being done in the mod_RSsimDLG.cpp program
   ...
   CString addrFormat;
      ...
      DWORD addrEndItem, offset = 0;
         ...
         { // modbus
            ...
            {
			//   Changed to always display 6-digit addresses by DL on 2014-12-28
			//   This fixes the address field having 49991-50000 followed by 410001-410010
         ...
         //sprintf(szString, addrFormat, offset+absItemNumber, offset+addrEndItem); // Orignial Code
		 if (!IsAddressFormatHex())                                                 // If added to show Modbus Inputs
         sprintf(szString, "%06d-%06d", offset+absItemNumber, offset+addrEndItem);  // Ex: 000001 - 065535
		 else                                                                       // with leading zeroes by DL
		 sprintf(szString, addrFormat, offset+absItemNumber, offset+addrEndItem);   // on 2015-01-04
