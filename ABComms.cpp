...
CString description;
   ...
   {
   //COMMTIMEOUTS timeout;  // Commented out by DL on 2016-10-16
   //BOOL err;              // Commented out by DL on 2016-10-16
   //DWORD reason;          // Commented out by DL on 2016-10-16
      ConfigurePort(pGlobalDialog->m_baud, pGlobalDialog->m_byteSize, pGlobalDialog->m_parity,
		            pGlobalDialog->m_stopBits, pGlobalDialog->m_rts, (NOPARITY==parity?FALSE:TRUE), longTimeouts);
      //set the timeouts shorter

	  /* Comment this and let the RS232Port.cpp handle the timeouts. It is called just above this. by DL on 2016-10-16
      ...
      reason = GetLastError();
	  */ // End of Comment by DL on 2016-10-16
   }
...
{
   if (stationID>=0 && stationID<STATIONTICKBOXESMAX)	// 2015-07-26 by DL added "=" for Active Station #0
   {
...
DWORD wordIndex;
   ...
   if (!dat.Open("ABDATA.DAT", CFile::modeWrite | CFile::shareExclusive | CFile::modeCreate, &ex) )
   {
      ...
      TCHAR szError[1024];
      ex.GetErrorMessage(szError, 1024);
      OutputDebugString( "Couldn't open destination file: ");  // Changed source to destination 2015-12-19 by DL
      OutputDebugString( szError);
